...
{
        PINSEL0 |= 3<<6;                        //P0.3 is selected for switch to raise an external interrupt as it supports EINT1 in function 4.
...
int main()
{
        ...
        IODIR0 =1<<25|1<<12|1<<15;      // buzzer 25, device1 12, device2 15.
        intialize_LCD();
        intialize_kpm();                                      //Initializing all the requirements
        InitUART0();
        Init_SPI();
        eint1_enable();                 // External interrupt enabling
        //spi_eeprom_pagewrite(0x0000,"2408",4); //this line is for storing the password into the EEPROM intially,firstly uncomment this line and execute only this line,after that comment this line and execute the other lines
        ...
        IOCLR0 = 1<<12|1<<15;           //Initially clearling all the devices
        cmdLCD(0x01);
        cmdLCD(0x80);
        strLCD("Enter the password:");
        delay_s(1);
                ...
                while(r_flag==0);          //process will wait untill you touch the touch screen
                        ...
                        if((xint<11) && (yint<14 && yint>=8) && (zint<15)) //Co-ordinates region selected for Fan       ON
                        {
                                if(enable==1)                            //will work only if touch is enabled
                                {
                                        ...
                                        IOSET0 = 1<<12;                  //If touched LED will be high
                                }
                        }
                        if((xint>11) && (yint<14 && yint>=8) && (zint<15)) //Co-ordinates region selected for Fan OFF
                        {
                                if(enable==1)                            //Will work only if touch is enabled
                                {
                                   ...
                                   IOCLR0 = 1<<12;                       //If Touched LED will be low
                                }
                        }
                        if((xint<11) && (yint<8) && (zint<15))   //Co-ordinates region selected for Light ON
                        {
                                if(enable==1)                            //Will work only if touch is enabled
                                {
                                        ...
                                        IOSET0 = 1<<15;                  //If Touched LED will be high
                                }
                        }
                        if((xint>11) && (yint<8) && (zint<15))   //Co-ordinates region selected for Light OFF
                        {
                                if(enable==1)                            //Will work only if touch is enabled
                                {
                                        ...
                                        IOCLR0 = 1<<15;                  //If Touched LED will be low
                                }
                        }
                        cmdLCD(0x01);
                        if(enable==1)
